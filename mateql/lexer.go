...
// Fork from https://github.com/VictoriaMetrics/metricsql/blob/5f227b4bf5679c28c59362aad96d196a8e4a986b/lexer.go#L204
// Modify for support graphite Target.
...
scanIdentLabel:
	for i < len(s) {
		...
		if isExtendGraphiteIdentChar(s[i]) {
			if !isGraphite && guess < 0 {
				guess = i
			}
			i++
			continue
...
// Function for support graphite target
// https://graphite.readthedocs.io/en/latest/render_api.html#paths-and-wildcards
// https://github.com/golang/go/blob/release-branch.go1.14/src/path/filepath/match.go#L18

func isExtendGraphiteIdentChar(ch byte) bool {
	return ch == '-' || ch == '|' || ch == '?'
}

func scanCharRange(s string) int {
	if len(s) == 0 || s[0] != '[' {
		return -1
	}
	// Ignore graphite target mix duration. eg. a.b.c[5m]
	if scanDuration(s[1:], true) > -1 {
		return -1
	}
	i := 1
	charCount := 0
...
func scanValueList(s string) int {
	if len(s) < 2 || s[0] != '{' {
		return -1
	}
	i := 1
scanValueListLabel:
	for i < len(s) {
		switch {
		case isIdentChar(s[i]) || isSpaceChar(s[i]) || s[i] == '-':
			i++
		case s[i] == '\\':
			// Do not verify the next char, since it is escaped.
			i += 2
