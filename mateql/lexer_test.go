...
func TestScanGraphiteIdent(t *testing.T) {
	f := func(s, resultExpected string) {
		t.Helper()
		result := scanIdent(s)
		if result != resultExpected {
...
func TestScanCharRange(t *testing.T) {
	f := func(s string, resultExpected int) {
		t.Helper()
		result := scanCharRange(s)
		if resultExpected == 0 {
			resultExpected = len(s)
		}
		if result != resultExpected {
			t.Fatalf("unexpected result for scanIdent(%q): got %d; want %d", s, result, resultExpected)
		}
	}
	f("[", -1)
	f("]", -1)
